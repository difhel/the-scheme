...
def test_builtin_types():
    """
    Testing the conversion of built-in types
    """
...
def test_simple_custom_types():
    """
    Testing the conversion of simple custom types
    """
    ...
    # test that if we have two constructors for one type
    # (User.user and User.bot) we should specify exactly
    # what constructor we want to use
    assert bot_unused_type.convert_to_scheme() == (
        "botUnused id: int, first_name: null | str, "
        "bot_creator: User = UnusedType"
    )
    ...
    assert bot_used_type.convert_to_scheme() == (
        "bot id: int, first_name: null | str, "
        "bot_creator: User.user = User"
    )
    BS._BSMeta__force_clear()  # pylint: disable = protected-access

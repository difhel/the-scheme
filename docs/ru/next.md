...
> **Внимание!**
> Вы смотрите проект документации к BS. В текущем виде функционал НЕ реализован и его не получится использовать.
...
```
- `1 <alignment>` - метод вернул `null`
- `0 ...` - метод вернул не `null`, далее идет описание объекта типа `User`.
...
```
(<namespace>::)? <type name> (.<constructor name>)>? <generics>? <dependencies>?
```
...
- **`<dependencies>`** - опциональная часть - в квадратных скобках перечислены переменные, от которых зависит данный тип, см. [Зависимые типы](./next.md#зависимые-типы).
